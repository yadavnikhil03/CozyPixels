  ...
  useEffect(() => {
    const handleMouseMove = (e) => {
      requestAnimationFrame(() => {
        ...
        setCoords(depths.map((depth) => ({ x: depth * x, y: depth * y })));
      });
    };
...
const Cactus = ({ x, y, scale, className = '' }) => (
  <g
    ...
    style={{ transform: `translate(${x}px, ${y}px) scale(${scale})`, transformOrigin: 'center', transformBox: 'fill-box' }}
  >
    <path
      fill="#0e7490"
...
export default function NotFoundPage({
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, backgroundColor: 'var(--page-bg)', overflow: 'hidden' }}>
      <div style={{ position: 'absolute', left: 0, top: 0, right: 0, bottom: 0 }}>
        <svg viewBox="0 0 2000 720" style={{ width: '100%', height: '100%', display: 'block', backgroundColor: '#374151' }} preserveAspectRatio="xMidYMid slice">
          <path style={{ transform: `translate(${p3.x}px,${p3.y}px)` }} strokeWidth={5} stroke="#1f2937" fill="#4b5563" d="m1831 198l-8 565l-95 3v-576.3zm-441-42v633.1h-257v-622.1zm-340 36v597.3h-201.7v-596.3zm-246 20v531.7h-53v-534.7zm-136-20v575.1h-153.4v-576.3zm-348 3v574.7h-159v-566.8z" />

          <path style={{ transform: `translate(${p3.x}px,${p3.y}px)` }} fill="#111827" d="m-203.5 227v-467.6h2433.1v553.6l-399.6-71l-102 29l-335-76l-258 80l-85-30l-202 32l-45-35l-50 19l-84-35l-154 61l-194-58l-160 58z" />
          ...
          <path style={{ transform: `translate(${p3.x}px,${p3.y}px)`, filter: 'blur(80px)' }} fill="#1f2937" d='m-300,400 H2400 V700 H0 z' />

          <text style={{ transform: `translate(${p4.x}px,${p4.y}px)`, filter: 'drop-shadow(0 0 50px #1f2937)' }} x='1000' y='550' textAnchor='middle' fill="#6b7280" fontSize="660px" fontWeight="800">{errorCode}</text>

          <path style={{ transform: `translate(${p5.x}px,${p5.y}px)` }} fill="#111827" d="m2195 396v531.1h-2437.2v-538.1l359.2 60l96-22l63 44l169-40l83 39l348-47l147 28l125-32l75 47l75-21l221 28l263-75l109 31z" />
      ...
      <div style={{ position: 'absolute', zIndex: 2, left: '50%', top: '8vh', transform: 'translateX(-50%)', width: 'min(720px, 86vw)', padding: '18px' }}>
        <h1 style={{ color: 'var(--color-on-surface)', fontSize: 'clamp(2rem, 6vw, 4.5rem)', marginBottom: '0.5rem', fontFamily: 'Pridi, serif', textAlign: 'center' }}>{title}</h1>
        <p style={{ color: 'var(--lightbox-info-muted)', fontWeight: 500, letterSpacing: '0.05rem', textAlign: 'center', marginBottom: '0.5rem' }}>{subtitle}</p>
        <p style={{ color: 'var(--lightbox-info-muted)', textAlign: 'center' }}>You can <a href="/" style={{ color: '#06b6d4', textDecoration: 'underline' }}>{homeText}</a>, or <span style={{ color: '#06b6d4' }}>{searchText}</span>.</p>
      </div>
    </div>
  );
}
